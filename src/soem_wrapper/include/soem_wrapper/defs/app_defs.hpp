...
struct DJIRC {
    ...
    read(const uint8_t* buf, int* offset, const std::string& prefix) {
        ...
        if (buf[*offset + 18]) {
            // channel
            custom_msgs_readdjirc_shared_msg.right_x = get_percentage((buf[*offset + 0] | buf[*offset + 1] << 8) & 0x07ff);
            custom_msgs_readdjirc_shared_msg.right_y = get_percentage((buf[*offset + 1] >> 3 | buf[*offset + 2] << 5) & 0x07ff);
            custom_msgs_readdjirc_shared_msg.left_x  = get_percentage((buf[*offset + 2] >> 6 | buf[*offset + 3] << 2 | buf[*offset + 4] << 10) & 0x07ff);
            custom_msgs_readdjirc_shared_msg.left_y  = get_percentage((buf[*offset + 4] >> 1 | buf[*offset + 5] << 7) & 0x07ff);
            custom_msgs_readdjirc_shared_msg.dial    = get_percentage((buf[*offset + 16] | buf[*offset + 17] << 8) & 0x07ff);
        ...
        for (int i = 1; i <= 4; i++) {
            if (const std::string motor_arg_prefix = fmt::format("{}sdowrite_motor{}_", prefix, i);
                ...
                case DJIMOTOR_CTRL_TYPE_SPEED: {
                    memcpy(
                        buf + *offset,
                        sdo_data.build_buf(motor_arg_prefix, {"control_type", "speed_pid_kp", "speed_pid_ki",
    ...
    init_sdo(uint8_t* buf, int* offset, const uint32_t& /*sn*/, const uint8_t slave_id,
        ...
        *offset += 8;
        ...
        case LK_CTRL_TYPE_MULTI_ROUND_POSITION_WITH_SPEED_LIMIT: {
            node->create_and_insert_subscriber<custom_msgs::msg::WriteLkMotorMultiRoundPositionControlWithSpeedLimit>(
                prefix, slave_id);
            break;
        }
        ...
        case LK_CTRL_TYPE_SINGLE_ROUND_POSITION_WITH_SPEED_LIMIT: {
            node->create_and_insert_subscriber<custom_msgs::msg::WriteLkMotorSingleRoundPositionControlWithSpeedLimit>(
                prefix, slave_id);
            break;
        }
        default: {}
